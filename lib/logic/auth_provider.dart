...
class AuthNotifier extends StateNotifier<AuthState> {
  ...
  Future<void> signInWithGoogle() async {
    ...
    try {
      ...
      if (isMobile) {
        const webClientId = '1516941054-u6ngjhp7qrh583k9hhs2vts8569l63uf.apps.googleusercontent.com'; 
        const iosClientId = 'YOUR_IOS_CLIENT_ID_FROM_GOOGLE_CLOUD.apps.googleusercontent.com'; 
  ...
  Future<void> signInWithSolana(BuildContext context) async {
        ...
        return SafeArea(
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                const Text("SELECT WALLET", style: TextStyle(color: Colors.cyanAccent, fontWeight: FontWeight.w900, fontSize: 18, letterSpacing: 1.5)),
                const SizedBox(height: 20),
                ListTile(
                  leading: const Icon(Icons.account_balance_wallet, color: Colors.purpleAccent),
                  title: const Text("Phantom Wallet", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                ...
                ListTile(
                  ...
                  title: const Text("Solflare Wallet", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                  tileColor: Colors.black26,
                  shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10), side: const BorderSide(color: Colors.white12)),
    ...
    try {
      ...
      if (selectedWallet == 'phantom') {
        walletUrl = Uri.parse('https://phantom.app/ul/v1/connect?app_url=https://terminal.online&redirect_link=$_getRedirectUrl');
        fallbackDownloadUrl = Uri.parse('https://phantom.app/');
      } else {
        walletUrl = Uri.parse('https://solflare.com/ul/v1/connect?app_url=https://terminal.online&redirect_link=$_getRedirectUrl');
      ...
      if (await canLaunchUrl(walletUrl)) {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text("${selectedWallet == 'phantom' ? 'Phantom' : 'Solflare'} successfully linked!"), backgroundColor: Colors.greenAccent)
          );
        }
      } else {
        debugPrint("No wallet detected. Routing to download page.");
        await launchUrl(fallbackDownloadUrl, mode: LaunchMode.externalApplication);
      }
